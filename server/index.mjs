...
app.get('/api/ncm/audio', async (q,r)=>{ try{ const id=String(q.query.id||''); if(!/^\d+$/.test(id)) return r.status(400).json({ok:false,error:'bad id'}); const su=await ncm.song_url_v1({ id, level:'standard', cookie:ncmCookie }); const u=su.body&&su.body.data&&su.body.data[0]; let url=u&&u.url||''; if(url) url=url.replace(/^http:/,'https:'); if(!url) return r.status(404).json({ok:false,error:'no audio url'}); await assertPublicUrl(url); const dl=await fetchCapped(url,{ maxBytes:80*1024*1024, timeoutMs:90000, headers:{'User-Agent':'Mozilla/5.0'} }); const ct=String(dl.ct||'').toLowerCase(); const type=(ct.includes('audio')||ct.includes('mpeg')||ct.includes('octet-stream'))?dl.ct:'audio/mpeg'; r.set('Content-Type',type); r.set('Cache-Control','private, max-age=600'); r.set('Content-Length',String(dl.buf.length)); r.send(dl.buf); }catch(e){ r.status(502).json({ok:false,error:String(e.message||e)}); } });
